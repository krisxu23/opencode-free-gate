...
| **对冲竞速** | 出口按近期表现评分排序、分批出发（首批少量、迟迟无人交付首个数据块再加发）；出口轮流分散到主上游与各镜像，一笔请求同时探多个镜像的队列；最快交付真实数据的出口胜出，赢家出现后立即取消其余在途请求 |
| **空流拦截** | 返回 200 却不带任何数据即关闭的流在网关内部被拦下并自动换出口，客户端不会收到“断流无 finish_reason”；中途夭折的出口/镜像会被记账降权，连续失败的节点自动暂停参与竞速（坐板凳），探活通过即恢复 |
...
| **图形界面** | 双击即用；状态 / 设置 / 日志三页，顶部彩色状态行一眼读出端口·出口·模型·竞速与健康度，配置存 exe 同目录 `config.json` |
